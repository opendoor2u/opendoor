...
*Setup instructions for a Gitian build of Opendoor Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Opendoor
...
- [Building Opendoor Core](#building-opendoor-core)
...
Clone the git repositories for Opendoor Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/opendoor2u/opendoor
```
...
in the Opendoor Core repository under 'Fetch and build inputs' to install sources which require
...
To build Opendoor Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Opendoor Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/opendoor/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/opendoor2u/opendoor
...
For example:
```bash
URL=https://github.com/crowning-/opendoor.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit opendoor=${COMMIT} --url opendoor=${URL} ../opendoor/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit opendoor=${COMMIT} --url opendoor=${URL} ../opendoor/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit opendoor=${COMMIT} --url opendoor=${URL} ../opendoor/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the opendoor git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../opendoor/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/opendoor.git
SIGPATH=/some/root/path/opendoor-detached-sigs.git

./bin/gbuild --url opendoor=${BTCPATH},signature=${SIGPATH} ../opendoor/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/opendoor-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/opendoor-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/opendoor-osx-build.assert
```
...
[opendoor/gitian.sigs](https://github.com/opendoorpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
