...
class WalletTest (BitcoinTestFramework):
    ...
    def check_fee_amount(self, curr_balance, balance_with_fee, fee_per_byte, tx_size):
        ...
        if fee < target_fee:
            raise AssertionError("Fee of %s OPENDOOR too low! (Should be %s OPENDOOR)"%(str(fee), str(target_fee)))
        # allow the node's estimation to be at most 2 bytes off
        if fee > fee_per_byte * (tx_size + 2):
            raise AssertionError("Fee of %s OPENDOOR too high! (Should be %s OPENDOOR)"%(str(fee), str(target_fee)))
        return curr_balance
    ...
    def run_test (self):
        ...
        # Send 210 OPENDOOR from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 1000 OPENDOOR in block rewards plus fees, but
        ...
        # Send 100 OPENDOOR with subtract fee from amount
        ...
        # Sendmany 100 OPENDOOR with subtract fee from amount
