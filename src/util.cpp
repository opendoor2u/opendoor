...
const char * const BITCOIN_CONF_FILENAME = "opendoor.conf";
const char * const BITCOIN_PID_FILENAME = "opendoord.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "opendoor" is a composite category enabling all Opendoor-related debug output
            if(ptrCategory->count(string("opendoor"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\OpendoorCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\OpendoorCore
    // Mac: ~/Library/Application Support/OpendoorCore
    // Unix: ~/.opendoorcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OpendoorCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/OpendoorCore";
#else
    // Unix
    return pathRet / ".opendoorcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty opendoor.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override opendoor.conf
