...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total opendoor balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized opendoor balance of the wallet\n"
...
UniValue debug(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "opendoor|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use comma to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "opendoor")
            + HelpExampleRpc("debug", "opendoor,net")
        );
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"opendooraddress\"\n"
            "\nReturn information about the given opendoor address.\n"
            "\nArguments:\n"
            "1. \"opendooraddress\"     (string, required) The opendoor address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"opendooraddress\", (string) The opendoor address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Opendoor address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are opendoor addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) opendoor address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"opendooraddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"opendooraddress\"  (string, required) The opendoor address to use for the signature.\n"
